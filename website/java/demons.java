...
class Character {
  ...
  /**
   		 ...
   		 *         2 - overcolliding
   		 */
  public Integer collision(int x, int y) {
...
class Tree {
  ...
  Tree(float maxHeight, Point root, Integer windiness, Integer strength, PGraphics canvas, Integer layer) {
    ...
    if (layer == 1) {
      ...
      branches.add(new Branch(this, root, random(70, 80), PI * 0.8f, strength, colour));
    } 
    else {
  ...
  public void tick() {
    ...
    if (drawing) {
      ...
      if (currentLength < finalLength) {
        ...
        rotate((float) angle);
        tree.canvas.ellipse(start.x, start.y, (float) (strength * 0.9f), (float) (strength * 0.9f));
        ...
        if (tree.depth < tree.maxDepth) {
          ...
          // Add new branches
          tree.branches.add(new Branch(tree, current, finalLength * 0.78f, (angle + random(0.4f, 0.8f)), strength, colour));
...
public void getCommand(ArrayList<Color> commands) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "demons" });
  }
}
